...
def tap(pointX, pointY):
    ...
    os.system("adb -s 127.0.0.1:62025  shell input swipe %s %s %s %s  %s" % (
        pointX, pointY, pointX, pointY, random.randrange(80, 200)))


def input(str):
    os.system("adb -s 127.0.0.1:62025  shell input text %s" % (str))


#  3  home   4 back
def tapKey(keyevent):
    os.system("adb -s 127.0.0.1:62025  shell input keyevent  %s" % (keyevent))
